...
###   This script attempts to download the signature file SHA256SUMS.asc from outbit.org
...
WORKINGDIR="/tmp/outbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://outbit.org/bin/"
VERSIONPREFIX="outbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'outbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Outbit Core binary release signing key installed?"
   fi
