...
{
    std::cout <<
        "This program outputs Outbit addresses and private keys from a wallet.dat file" << std::endl
        << std::endl
        << "Usage and options: "
        << std::endl
...
std::string WalletUtilityDB::getAddress(CDataStream ssKey)
{
    ...
    std::string strAddr = COutbitAddress(id).ToString();
...
std::string WalletUtilityDB::getKey(CDataStream ssKey, CDataStream ssValue)
{
    ...
    if (key.Load(pkey, vchPubKey, true))
        strKey = COutbitSecret(key).ToString();
...
std::string WalletUtilityDB::getCryptedKey(CDataStream ssKey, CDataStream ssValue, std::string masterPass)
{
    ...
    std::string strKey = COutbitSecret(key).ToString();
    return strKey;
}
