Outbit Core version 0.11.0 is now available from:

  <https://outbit.org/bin/outbit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Outbit Core or other software:
...
For example, add the following to `outbit.conf`:
...
related to the blockchain in the outbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Outbit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Outbit Core will 
...
For an example script see [outbit-submittx](https://github.com/laanwj/outbit-submittx).
...
- #5334 `cf87536` liboutbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new outbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only OutbitAmountField
- #5753 `0f44672` Add outbit logo to about screen
...
- #5809 `46bfbe7` Add outbit-cli man page
...
- #5849 `41259ca` contrib/init/outbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Outbit-Qt.app to Outbit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for outbit-tx)
