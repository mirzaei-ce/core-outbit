...
*Setup instructions for a Gitian build of Outbit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Outbit
...
- [Building Outbit](#building-outbit)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-outbit-host/README.md).
...
Clone the git repositories for outbit and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/outbit/outbit
```
...
in the outbit repository under 'Fetch and build inputs' to install sources which require
...
To build Outbit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the outbit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/outbit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/outbit/outbit
...
For example:
```bash
URL=https://github.com/laanwj/outbit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit outbit=${COMMIT} --url outbit=${URL} ../outbit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit outbit=${COMMIT} --url outbit=${URL} ../outbit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit outbit=${COMMIT} --url outbit=${URL} ../outbit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the outbit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../outbit/contrib/gitian-descriptors/*|sort|uniq )
...
UBTPATH=/some/root/path/outbit.git
SIGPATH=/some/root/path/outbit-detached-sigs.git

./bin/gbuild --url outbit=${UBTPATH},signature=${SIGPATH} ../outbit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/outbit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/outbit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/outbit-osx-build.assert
```
...
[outbit/gitian.sigs](https://github.com/outbit/gitian.sigs/) repository, or if that's not possible create a pull
